...
sb_s32 init_xdr_socket()
{
	...
	sin.sin_addr.s_addr = htonl(g_device_config.xdr_dev_ip);
	sin.sin_port = htons(g_device_config.xdr_dev_port);

	printf("connecting %s at port %hu\n", inet_ntoa(sin.sin_addr),
			ntohs(sin.sin_port));
...
ssize_t send_xdr_pkt(const void *buf, size_t size)
{
	size_t sent = 0;
	ssize_t ret;
...
ssize_t recv_xdr_pkt(void *buf, size_t size)
{
	size_t received = 0;
	ssize_t ret;
